...
เมื่อเปิด `DrawRangeLines` / `DrawSignals` EA จะมาร์คทุกเหตุการณ์ของระบบไว้บนกราฟ
(เก็บของวันก่อน ๆ ไว้ให้ย้อนดูได้ และลบทั้งหมดอัตโนมัติเมื่อถอด EA ออกจากกราฟ):

| สัญลักษณ์ | สี | ความหมาย |
|---|---|---|
| เส้นทึบ + ป้าย `OR High` / `OR Low` | เขียว / ส้มแดง | กรอบ Opening Range จากแท่ง M5 แรก |
| ลูกศร + `BREAK` | น้ำเงิน | แท่ง M1 ที่ปิดทะลุกรอบด้วยเนื้อเทียน |
| วงกลม + `RETEST` | เหลืองทอง | แท่งที่ราคาย้อนกลับมาแตะโซนเส้นที่ทะลุ |
| ลูกศร + `CONFIRM` | เขียว (Buy) / แดง (Sell) | แท่งคอนเฟิร์มเนื้อเทียนใหญ่ที่เข้าเงื่อนไข |
| ลูกศร Buy/Sell + ป้าย `BUY/SELL lot @ ราคา` | เขียว / แดง | จุดเปิดออเดอร์จริง |
| เส้นประ + ป้าย `SL` / `TP` | แดง / เขียว | ระดับ Stop Loss และ Take Profit ของไม้นั้น |
| กากบาท + `CANCEL` | เทา | setup ถูกยกเลิก (ราคาปิดทะลุกลับไปอีกฝั่งของกรอบ) |
